...
class Contact extends React.Component {
	...
	onSubmit = e => {
		e.preventDefault();
		this.state.nameAlert || this.state.emailAlert ||
		this.state.subjectAlert || this.state.messageAlert ||
				...
				<form onSubmit={this.onSubmit}>
					...
					<div className="input-container">
						<input className={'subject'} type={'text'} value={this.state.subject} onChange={this.handleChange}
									 placeholder={'W czym możemy pomóc'} name={'subject'} required/>
						{this.state.subjectAlert && <span>Temat musi posiadać conajmniej 6 znaki</span>}
					</div>
					<div className="input-container">
						<textarea value={this.state.message} name={'message'} onChange={this.handleChange}
											placeholder={'Twoja wiadomość'}/>
						{this.state.messageAlert && <span> Wiadomość musi posiadać conajmniej 20 znaki</span>}
					</div>
