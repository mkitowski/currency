...
class App extends React.Component {
  ...
  UserLogin = () => {
    ...
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        this.state.db
          .doc(`${this.state.userInfo.Login.email}/history`)
          .get()
          .then((el) => {
            this.setState({
              userInfo: {
  ...
  setBase(NBPinput, code) {
    ...
    this.setState({
      //set base state for rate from NBP
      [code + '-base']: {
        bid,
        ask,
        spread,
  ...
  generateHistory(real, code) {
    ...
    for (let i = 0; i < 720; i++) {
      ...
      newBid =
        Math.floor(newBid * 1000 + Math.floor(Math.random() * (5 + 5) - 5)) /
        1000;
  ...
  simulateChanges(real) {
    //Simulation of life rates changes
    const date = new Date();
    const newBid =
      Math.floor(real.bid * 1000 + Math.floor(Math.random() * (45 + 45) - 45)) /
      1000;
    const newRate = {
      code: real.code,
      hour: date.getHours(),
    ...
    this.setState({
      [real.code + '-day']: [...this.state[real.code + '-day'], newRate].slice(
        1,
      ),
  ...
  getData() {
    //getting current rates for defined currencies, generating 24h history, simulating changes
      ...
      <BrowserRouter>
        <MainDiv>
          <Header
            ...
            userLogged={this.state.userInfo.Login.logged}
          />
          <Switch>
            <Route
              exact
            ...
            <Route
              exact
              path="/history"
              render={(props) => <History {...props} data={this.state} />}
            />
            <Route
              exact
              path="/contact"
              render={(props) => (
                <Contact {...props} data={this.state.userInfo.Login} />
              )}
            />
